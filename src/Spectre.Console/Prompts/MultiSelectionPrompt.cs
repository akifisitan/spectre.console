    ...
    where T : notnull
{
    ...
    public async Task<List<T>> ShowAsync(IAnsiConsole console, CancellationToken cancellationToken)
    {
        ...
        var result = await prompt.Show(Tree, converter, Mode, false, false, PageSize, WrapAround, searchFilter: null, cancellationToken: cancellationToken).ConfigureAwait(false);
